...
//				"c.class_date ,10,c.regist_num,c.prof_name");	
	...
	public ArrayList<CourseDto> CourseAdmission() throws SQLException {
		try {
			conn = DbConnection.getConnection();
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public ArrayList<CourseDto> CourseAllow() throws SQLException {
		try {
			conn = DbConnection.getConnection();
		} catch (Exception e) {
			// TODO Auto-generated catch block
		...
		CourseDto member= null;
		pstmt = conn.prepareStatement(
				"select k.course_name , c.start_date , c.end_date , count(c.course_list), c.course_list " +
				"from course c JOIN course_kind k on c.course_list=k.course_num "+
				"group by k.course_name, c.start_date, c.end_date,c.course_list " +
				"order by k.course_name");
		rs = pstmt.executeQuery();
		while (rs.next()) {
			member = new CourseDto();
			member.setCourse_list2(rs.getString(1)); // 교육과정분류
			member.setStart_date(rs.getDate(2)); // 교육시작일
			member.setEnd_date(rs.getDate(3)); // 교육종료일
			member.setTraining_list(rs.getInt(4));
			member.setCourse_list(rs.getInt(5));
			list.add(member);
		}
		return list;
	}
